...
def get_embeddings(question):
    client = InferenceClient(
        model="BAAI/bge-large-en-v1.5", token=st.secrets["HF_API_TOKEN"]
    )
...
def run():
    ...
    question = st.text_input(
        ...
        placeholder="e.g. What is the doctrine of justification?",
    )
    ...
    if isinstance(response, str):
        ...
        st.markdown(
            "Please provide feedback [here](https://docs.google.com/forms/d/e/1FAIpQLSdiQ5t111iM4-s7o3LraaLjHTsNLCY5vnYA-f38fXVVcgQKug/viewform?usp=sf_link)"
        )
