...
def document_chunking(document, minimum_chunk_size=None):
    ...
    for sentence in sentences:
        ...
        if len(chunk) < minimum_chunk_size and len(chunk + sentence) < 2 * minimum_chunk_size:
            chunk += sentence
        # Otherwise store chunk and reset chunk
            ...
            if len(sentence) > 2 * minimum_chunk_size:
                sentence_split = [sentence[i:i+minimum_chunk_size] for i in range(0, len(sentence), 2 * minimum_chunk_size)]
...
if __name__ == '__main__':
    ...
    dataset_full.save_to_disk(directory + 'theology_qa')
    dataset_full.push_to_hub('hxyue1/theology_qa')
    huggingface_hub.create_tag('hxyue1/theology_qa', tag='0.1', repo_type='dataset')
